...
for bit_index = bit_count-1:-1:1
    ...
    for transition_index = 1:transition_count
        if(set(transitions(transition_index,1)))
            betas(transitions(transition_index,1),bit_index) = max(betas(transitions(transition_index,1),bit_index), (betas(transitions(transition_index,2),bit_index+1) + gammas(transition_index, bit_index+1)));
        else
