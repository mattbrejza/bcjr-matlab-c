        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    if ((strncmp(input_buf,"uec",5)==0) || (strncmp(input_buf,"new",5)==0))
    {
        ...
        if(mxGetM(prhs[3])==1 || mxGetM(prhs[3])>159) {
            mexErrMsgIdAndTxt("MyToolbox:arrayProduct:notMatrix","transitions must be Tx5 (one codeword), Tx6 (two codewords) or Tx7 (two codewords).  Max transitions = 160");
        }
